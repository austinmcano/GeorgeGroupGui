...
class QCM_view(QtWidgets.QDockWidget):
    ...
    def qcm_mass_hc(self):
        ...
        if self.ui.comboBox_2.currentText() == 'Half+Full Cycle':
            ...
            ApplicationSettings.ALL_DATA_PLOTTED['Full Cycle'] =self.main_window.ax.plot(mc_f)
            ApplicationSettings.ALL_DATA_PLOTTED['Half Cycle A'] =self.main_window.ax.plot(mc_a)
            ApplicationSettings.ALL_DATA_PLOTTED['Half Cycle B'] =self.main_window.ax.plot(mc_b)
        elif self.ui.comboBox_2.currentText() == 'Density Half+Full Cycle':
            pass
        elif self.ui.comboBox_2.currentText() == 'Plot Mass':
            ApplicationSettings.ALL_DATA_PLOTTED['Mass'] = self.main_window.ax.plot(data[0],data[2])
    ...
    def qcm(self):
        ...
        if self.ui.comboBox.currentText() == 'Half+Full Cycle':
            ApplicationSettings.ALL_DATA_PLOTTED['MC_A'] = self.main_window.ax.plot(mc_a)
            ApplicationSettings.ALL_DATA_PLOTTED['MC_B'] = self.main_window.ax.plot(mc_b)
            ApplicationSettings.ALL_DATA_PLOTTED['MC_F'] = self.main_window.ax.plot(mc_f)
            ...
            ApplicationSettings.ALL_DATA_PLOTTED['Mass'] = self.main_window.ax.plot(data[0], data[1])

        elif self.ui.comboBox.currentText() == 'Density Half+Full Cycle':
            ApplicationSettings.ALL_DATA_PLOTTED['MCD_A'] = self.main_window.ax.plot(hcd_a)
            ApplicationSettings.ALL_DATA_PLOTTED['MCD_B'] = self.main_window.ax.plot(hcd_a)
            ApplicationSettings.ALL_DATA_PLOTTED['MCD_F'] = self.main_window.ax.plot(fc_d)
        self.ui.total_ml_label.setText(str(np.sum(mc_f)))
        self.ui.ave_mcpahc_label.setText(str(np.average(mc_a)))
        self.ui.ave_mcpbhc_label.setText(str(np.average(mc_b)))
        self.ui.ave_mcpc_label.setText(str(np.average(mc_f)))
